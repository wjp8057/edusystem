...
class RoomOption extends MyService{
    ...
    function getList($page=1,$rows=20){
        ...
        $count= $this->query->table('roomoptions')->count();
        ...
        try {
            if (isset($postData["inserted"])) {
                ...
                foreach ($listUpdated as $one) {
                    ...
                    $data['name'] = $one->name;
                    $row = $this->query->table('roomoptions')->insert($data);
                    if ($row > 0)
                        $insertRow++;
                }
            }
            if (isset($postData["updated"])) {
                ...
                foreach ($listUpdated as $one) {
                    ...
                    $data['value'] = $one->value;
                    $updateRow += $this->query->table('roomoptions')->where($condition)->update($data);
                }
            }
            //删除部分
            if (isset($postData["deleted"])) {
                ...
                foreach ($listUpdated as $one) {
                    $condition = null;
                    $condition['name'] = $one->name;
                    $deleteRow += $this->query->table('roomoptions')->where($condition)->delete();
                }
            }
        }
        catch(\Exception $e){
