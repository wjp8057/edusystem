        ...
        try {
            if (isset($postData["inserted"])) {
                ...
                foreach ($listUpdated as $one) {
                    ...
                    $row = $this->query->table('statusoptions')->insert($data);
                    if ($row > 0)
                        $insertRow++;
                }
            }
            if (isset($postData["updated"])) {
                ...
                foreach ($listUpdated as $one) {
                    ...
                    $data['value'] = $one->value;
                    $updateRow += $this->query->table('statusoptions')->where($condition)->update($data);
                }
            }
            //删除部分
            if (isset($postData["deleted"])) {
                ...
                foreach ($listUpdated as $one) {
                    $condition = null;
                    $condition['name'] = $one->name;
                    $deleteRow += $this->query->table('statusoptions')->where($condition)->delete();
                }
            }
        } catch (\Exception $e) {
            $this->query->rollback();
