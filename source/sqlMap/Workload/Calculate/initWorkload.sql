...
CASE
  ...
  WHEN WORKTYPE='M9'
	...
	ELSE TOTAL END AS WORKLOAD FROM WORKLOADCALC AS W
WHERE WORKLOADCALC.ID=W.ID 
) WHERE [YEAR]=:YEAR AND TERM=:TERM AND COURSENO IN (SELECT COURSENO FROM COURSES WHERE SCHOOL LIKE :SCHOOL)