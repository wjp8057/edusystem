select count(*) [count],sum(seats) seats,sum(sxjunit) sxjunit,cast((sum(sxjunit)/count(*)) as decimal)*2 lyl from (
...
where j.time = t.name and year =:year and term =:term 
	and c.seats >=:seats_gt and c.seats<=:seats_lt and day<6 
