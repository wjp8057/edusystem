...
class ExamAction extends RightAction {
    ...
    public function qlist(){
        if($this->_hasJson){
            ...
            $bind = $this->model->getBind("STUDENTNO,YEAR,TERM",array(session("studnet_studentno"),intval($_REQUEST["YEAR"]), intval($_REQUEST["TERM"])));
    ...
    public function seat(){
        ...
        if($row==false){
            ...
            for($i=0; $i<count($data);$i+=3){
                for($ii =0; $ii<3; $ii++){
                    if(trim($data[$i+$ii]["STUDENTNO"])==session("S_USER_NAME")){
