...
class DbSqlsrv extends Db{
    ...
    public function connect($config='',$linkNum=0) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            $connectInfo  =  array('Database'=>$config['database'],'UID'=>$config['username'],'PWD'=>$config['password'],'CharacterSet' => C('DEFAULT_CHARSET'),'TransactionIsolation'=> C('TRANSACTION_ISOLATION'));
...
/*        print_r($bind);*/
    ...
    public function error($result = true) {
        ...
        foreach( $errors as $error ) {
            //$this->error .= $error['code'].':'.iconv("GBK","UTF-8", $error['message']);
            $this->error .= $error['code'].':'. $error['message'];
        }
        if('' != $this->queryStr){
            $this->error .= "\n [ SQL语句 ] : ".$this->queryStr." \n[PARAMS]: ".print_r($this->_sqlbind, true);
        }
