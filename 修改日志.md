...
##2016-11-1 新增毕业审核存储过程。
CREATE PROCEDURE [dbo].[graduate_audit]
AS
BEGIN
	--更新总学分
update studentplan
set credits=majorplan.mcredits
...
where  form!='4' and  exists   (select * from students where students.studentno=graduate.studentno and students.status in ('A','C','H'))
...
from graduate left join ( --课程同课号仅计算一次
	...
	select  distinct scores.studentno,courses.credits from scores  inner join courses on courses.courseno=scores.courseno
...
where graduate.form='4' and  exists(select * from students where students.studentno=graduate.studentno  and students.status in ('A','C','H') )
...
from graduate inner join r12 on r12.programno=graduate.programno
where form in ('1','2','3')  and  exists   (select * from students where students.studentno=graduate.studentno and students.status in ('A','C','H')) and
...
from graduate inner join r12 on r12.programno=graduate.programno
where form in ('1','2')  and  exists   (select * from students where students.studentno=graduate.studentno and students.status in ('A','C','H'))
	--有成绩记录
